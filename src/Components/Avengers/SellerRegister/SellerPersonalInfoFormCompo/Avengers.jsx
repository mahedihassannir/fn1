...
const Avengers = () => {
	...
	const handleimageWithnew = (e) => {

		// first 0 index need to get the image url
		const file = e.target.files[0]
		console.log(file);

		// make a new from data 

		const Imagedata = new FormData()

		// make a append file to create value

		Imagedata.append('image', file)
		// this image fetch is for the upload on the image bb upload
		fetch(uploadUrl, {
			method: "POST",
			body: Imagedata
		})
		...
		// TODO: This value change when the image uploaded in the image hosting service 
				...
				<form onSubmit={handleSubmit} className="">
					...
					<div className=" px-2 grid grid-cols-2 gap-2 md:grid-cols-3 md:w-11/12 mx-auto mt-2 md:mt-10">
						...
						<div className="mt-4">
							...
							<input className='block bg-blue-100 border-2 outline-none py-2 w-50 md:w-80  border-gray-400 text-black rounded  pl-2 focus:placeholder-green-500  font-semibold' type="email" name="" id="" required placeholder='ইমেইল ঠিকানা' />
						</div>
						{/* ends */}
						...
						<div className="mt-4">
							...
							<input className='block bg-blue-100 border-2 outline-none py-2 w-50 md:w-80  border-gray-400 text-black rounded  pl-2 focus:placeholder-green-500  font-semibold' type="text" name="" id="" required placeholder='Company Name' />
						</div>
						{/* ends */}
					...
					<div className="">
						...
						<div className="w-11/12 mx-auto pb-6 ">
							...
							<div onClick={handleimage} className="w-80 h-40">
								<h1>{image ? image.name : "Select Store name "}</h1>
								{
									image ?
