    ...
    return (
        <section>
            ...
            <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-x-5 gap-y-10 lg:mx-20 my-10'>
                {
                    faceProducts.map(face => <div key={face._id}>
                        <div className='hover:shadow-md hover:border-2 hover:border-gray-300 hover:ease-in-out cursor-pointer rounded-lg'>
                           <div className='relative'>
                           <img className='rounded-t-lg' src={face.image} alt="" />
                            <p className='absolute top-0 rounded-t-lg right-0 bg-orange-500  rounded-l-full text-white font-bold px-1'>-10%</p>
                           </div>
                           <div className='pl-2'>
                           <div className='h-14 overflow-hidden'>
                           <p className='overflow-hidden font-bold'>{face.name}</p>
                           </div>
                           <p className='font-bold mt-8 text-orange-500'><span className='font-extrabold'>৳</span>{face.price}</p>
                           <p className='line-through text-sm text-zinc-400'><span className='font-extrabold'>৳</span>{face.price}</p>
                           </div>
